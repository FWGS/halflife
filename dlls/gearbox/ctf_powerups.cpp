...
void CPowerupCTFBase::RuneTouch(CBaseEntity *pOther)
{
	...
	CGearboxPlayer* pGearboxPlayer = (CGearboxPlayer*)pOther;
	if (!pGearboxPlayer)
		return;

	//Only one per customer
	if (pGearboxPlayer->m_iRuneStatus)
	{
	...
	pGearboxPlayer->m_iRuneStatus = m_iRuneFlag; //Add me the rune flag
	...
	MESSAGE_BEGIN(MSG_ONE, gmsgRuneStatus, NULL, pOther->pev);
		WRITE_BYTE(pGearboxPlayer->m_iRuneStatus);
	MESSAGE_END();
...
void DropRune(CBasePlayer *pPlayer)
{
	...
	if (pGearboxPlayer->m_iRuneStatus == 0)
		return;
	...
	if (pGearboxPlayer->m_iRuneStatus == IT_CTF_ACCELERATOR_FLAG)
	{
	...
	else if (pGearboxPlayer->m_iRuneStatus == IT_CTF_BACKPACK_FLAG)
	{
	...
	else if (pGearboxPlayer->m_iRuneStatus == IT_CTF_REGEN_FLAG)
	{
	...
	else if (pGearboxPlayer->m_iRuneStatus == IT_CTF_LONGJUMP_FLAG)
	{
	...
	else if (pGearboxPlayer->m_iRuneStatus == IT_CTF_PORTABLEHEV_FLAG)
	{
	...
	pGearboxPlayer->m_iRuneStatus = 0;
