...
{
	return CGearboxMultiplay::ClientConnected(pEntity, pszName, pszAddress, szRejectReason);
}
...
{
	CGearboxPlayer* pGearboxPlayer = (CGearboxPlayer*)pPlayer;
	if (!pGearboxPlayer)
		return;
	...
	if (pGearboxPlayer->m_flNextTeamChange > gpGlobals->time)
		return;

	pGearboxPlayer->m_flNextTeamChange = gpGlobals->time + 5;
...
int CCTFMultiplay::TeamWithFewestPlayers(void)
{
	...
	while ((pPlayer != NULL) && (!FNullEnt(pPlayer->edict())))
	{
		...
		{
			player = GetClassPtr((CGearboxPlayer *)pPlayer->pev);
...
BOOL CCTFMultiplay::ClientCommand(CBasePlayer *pPlayer, const char *pcmd)
{
	...
	else if (FStrEq(pcmd, "changeteam"))
	{
		if (pPlayer->pev->team != 0)
		{
			pGearboxPlayer->ShowMenu(1 + 2 + 16 + 512, -1, FALSE, "#Team_Menu_Join_IG");
			pGearboxPlayer->m_iMenu = Team_Menu_IG;
		}
...
void CCTFMultiplay::PlayerTakeDamage(CBasePlayer *pPlayer, CBaseEntity *pAttacker)
{
	...
	if (pGearboxPlayer->m_bHasFlag)
	{
		pGearboxPlayer->pCarrierHurter = (CBasePlayer *)pAttacker;
		pGearboxPlayer->m_flCarrierHurtTime = gpGlobals->time + TEAM_CAPTURE_CARRIER_DANGER_PROTECT_TIMEOUT;
	}
...
void CCTFMultiplay::PlayerSpawn(CBasePlayer *pPlayer)
{
	...
	if (pPlayer->pev->team == 0)
	{
		...
		pGearboxPlayer->m_flFlagStatusTime = gpGlobals->time + 0.1;
	}
	else
	{
		...
		if (addDefault)
		{
			pGearboxPlayer->m_bHasFlag = FALSE;
...
void CGearboxPlayer::ShowMenu(int bitsValidSlots, int nDisplayTime, BOOL fNeedMore, char *pszText)
{
...
{
	CGearboxMultiplay::InitHUD(pPlayer);
	...
	for (int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		if (plr)
		{
			...
			if (((CGearboxPlayer *)plr)->m_bHasFlag)
			{
	...
	if (pPlayer->pev->team == 0)
	{
		pGearboxPlayer->ShowMenu(1 + 2 + 16, -1, FALSE, "#Team_Menu_Join");
		pGearboxPlayer->m_iMenu = Team_Menu;
	}
}
...
void CCTFMultiplay::DeathNotice(CBasePlayer *pVictim, entvars_t *pKiller, entvars_t *pevInflictor)
{
	...
	{
		CGearboxPlayer *pk = (CGearboxPlayer*)CBaseEntity::Instance(pKiller);
	...
	CGearboxMultiplay::DeathNotice(pVictim, pKiller, pevInflictor);
}
...
void CCTFMultiplay::ClientDisconnected(edict_t *pClient)
{
	if (pClient)
	{
		CGearboxPlayer *pPlayer = (CGearboxPlayer *)CBaseEntity::Instance(pClient);
...
void CCTFMultiplay::PlayerThink(CBasePlayer *pPlayer)
{
	...
	if (pGearboxPlayer->pFlagCarrierKiller)
	{
		if (pGearboxPlayer->m_flFlagCarrierKillTime <= gpGlobals->time)
			pGearboxPlayer->pFlagCarrierKiller = NULL;
	}

	if (pGearboxPlayer->pFlagReturner)
	{
		if (pGearboxPlayer->m_flFlagReturnTime <= gpGlobals->time)
			pGearboxPlayer->pFlagReturner = NULL;
	}

	if (pGearboxPlayer->pCarrierHurter)
	{
		if (pGearboxPlayer->m_flCarrierHurtTime <= gpGlobals->time)
			pGearboxPlayer->pCarrierHurter = NULL;
	}

	// If the player has backpack powerup, replenish his ammunition.
	if (pGearboxPlayer->m_iRuneStatus == IT_CTF_BACKPACK_FLAG)
	{
		if (pGearboxPlayer->m_flRegenTime <= gpGlobals->time)
		{
			int i;
			ItemInfo II;
			...
			{
				item = pGearboxPlayer->m_rgpPlayerItems[i];
				...
				if (item)
				{
					...
					if (item->GetWeaponPtr() && ((CBasePlayerItem*)item)->GetItemInfo(&II))
					{
						...
						if (weapon && weapon->m_iClip < II.iMaxClip)
						{
							...
							pGearboxPlayer->m_flRegenTime = gpGlobals->time + 1;
	...
	if (pGearboxPlayer->m_iRuneStatus == IT_CTF_PORTABLEHEV_FLAG)
	{
		if (pGearboxPlayer->m_flRegenTime <= gpGlobals->time)
		{
	...
	if (pGearboxPlayer->m_iRuneStatus == IT_CTF_REGEN_FLAG)
	{
		if (pGearboxPlayer->m_flRegenTime <= gpGlobals->time)
		{
	...
	if (pGearboxPlayer->m_flFlagStatusTime && pGearboxPlayer->m_flFlagStatusTime <= gpGlobals->time)
...
{
	CGearboxPlayer* pGearboxVictim = (CGearboxPlayer*)pVictim;
	if (!pGearboxVictim)
		return;

	CGearboxPlayer *pk = NULL;
	...
	if (pGearboxVictim->m_bHasFlag)
	{
		if (pk)
		{
			if (pVictim->pev->team != pk->pev->team)
			{
				if (pGearboxVictim->m_flCarrierPickupTime <= gpGlobals->time)
				...
				if (pk->pev->team == RED)
				{
					...
					if (iBlueFlagStatus == BLUE_FLAG_STOLEN)
					{
						for (int i = 1; i <= gpGlobals->maxClients; i++)
						{
							CGearboxPlayer *pTeamMate = (CGearboxPlayer *)UTIL_PlayerByIndex(i);
		...
		pGearboxVictim->m_bHasFlag = FALSE;
	...
	switch (pGearboxVictim->m_iRuneStatus)
	{
	case IT_CTF_ACCELERATOR_FLAG:
	...
	if (pGearboxVictim->m_iRuneStatus)
	{
		UTIL_LogPrintf("\"%s<%i><%s><%s>\" triggered \"Dropped_%s\"\n",
			STRING(pVictim->pev->netname),
			GETPLAYERUSERID(pVictim->edict()),
	...
	pGearboxVictim->m_iRuneStatus = 0;

	MESSAGE_BEGIN(MSG_ONE, gmsgRuneStatus, NULL, pVictim->pev);
	WRITE_BYTE(pGearboxVictim->m_iRuneStatus);
	MESSAGE_END();

	if (!m_DisableDeathPenalty)
	{
		CGearboxMultiplay::PlayerKilled(pVictim, pKiller, pInflictor);
		RecountTeams();
	}
}
...
BOOL CCTFMultiplay::FPlayerCanTakeDamage(CBasePlayer *pPlayer, CBaseEntity *pAttacker)
{
	...
	return CGearboxMultiplay::FPlayerCanTakeDamage(pPlayer, pAttacker);
}
...
BOOL CCTFMultiplay::ShouldAutoAim(CBasePlayer *pPlayer, edict_t *target)
{
	...
	return CGearboxMultiplay::ShouldAutoAim(pPlayer, target);
}
...
void CCTFMultiplay::GetFlagStatus(CBasePlayer *pPlayer)
{
	...
	if (pPlayer)
	{
		...
		pGearboxPlayer->m_flFlagStatusTime = 0.0;
	}
	else
	{
